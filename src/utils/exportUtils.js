...
function bannerOutcome(doc, match, startY) { 
  ...
  doc.text(`Punti tempi: USMA Padova ${vigPts} - ${oppPts} ${match.opponent}`, x + w - 6, y + 7, { align: 'right' });
...
function formatEventLine(e, opponentName) {
  ...
  if (t === 'missed-shot' || t === 'opponent-missed-shot') { const isVig = t === 'missed-shot'; return sanitize(`${min}Tiro fuori ${isVig ? (ps||'USMA Padova') : opp}`); }
  if (t === 'shot-blocked' || t === 'opponent-shot-blocked') { const isVig = t === 'shot-blocked'; return sanitize(`${min}${isVig ? (ps||'USMA Padova') : opp} tiro parato`); }
  if (t === 'save' || t === 'opponent-save') { const isVig = t === 'save'; return sanitize(`${min}Parata ${isVig ? (ps||'USMA Padova') : `portiere ${opp}`}`); }
  if (t?.includes('palo-') || t?.includes('traversa-')) { const isVig = e.team === 'vigontina'; const hit = e.hitType === 'palo' ? 'Palo' : 'Traversa'; return sanitize(`${min}${hit} ${isVig ? (ps||'USMA Padova') : opp}`); }

...
const loadLogoAsBase64 = async () => { const p = '/logo-usma.png'; try { const r = await fetch(p); if (!r.ok) return null; const b = await r.blob(); const rd = new FileReader(); return await new Promise((res)=>{ rd.onload=()=>res(rd.result); rd.onerror=()=>res(null); rd.readAsDataURL(b); }); } catch { return null; } };

function drawHeader(doc, match) { const pageW = doc.internal.pageSize.width; const logo = match.__logoBase64; if (logo) { try { doc.addImage(logo, 'PNG', MARGINS.left, 10, 22, 22); } catch {} } doc.setFont('helvetica','bold'); doc.setFontSize(14); doc.setTextColor(0,0,0); doc.text('USMA PADOVA - REPORT PARTITA', pageW/2, 18, { align: 'center' }); return 34; }
...
function bannerOutcome(doc, match, startY) { 
  ...
  const usmaPts = calculatePoints(match, 'usma');
  const oppPts = calculatePoints(match, 'opponent');

  // Gol totali (solo informativi), esclusa Prova Tecnica
  let uGoals=0,oGoals=0; nonTechnicalPeriods(match).forEach(p=>{ uGoals+=safeNum(p.usma); oGoals+=safeNum(p.opponent); });

  doc.setFont('helvetica','normal'); doc.setTextColor(50,50,50);
  doc.text(`Punti tempi: USMA Padova ${usmaPts} - ${oppPts} ${match.opponent}`, x + w - 6, y + 7, { align: 'right' });
  doc.setFontSize(9); doc.setTextColor(90,90,90);
  doc.text(`Gol: ${uGoals}-${oGoals}`, x + w - 6, y + 13, { align: 'right' });
  return y + h + 8; 
}
...
function infoBlock(doc, match, startY) {
  ...
  doc.text(`${match.teamName || 'USMA Padova'} vs ${match.opponent}`, MARGINS.left, y); y += 6;
...
function formatEventLine(e, opponentName) {
  ...
  if (t === 'own-goal') return sanitize(`${min}Autogol USMA Padova`);
  if (t === 'opponent-own-goal') return sanitize(`${min}Autogol ${opp}`);
  if (t.includes('penalty') && t.includes('missed')) { const who = t === 'penalty-missed' ? 'USMA Padova' : opp; return sanitize(`${min}Rigore fallito ${who}`); }
  if (t === 'missed-shot' || t === 'opponent-missed-shot') { const isUsma = t === 'missed-shot'; return sanitize(`${min}Tiro fuori ${isUsma ? (ps||'USMA Padova') : opp}`); }
  if (t === 'shot-blocked' || t === 'opponent-shot-blocked') { const isUsma = t === 'shot-blocked'; return sanitize(`${min}${isUsma ? (ps||'USMA Padova') : opp} tiro parato`); }
  if (t === 'save' || t === 'opponent-save') { const isUsma = t === 'save'; return sanitize(`${min}Parata ${isUsma ? (ps||'USMA Padova') : `portiere ${opp}`}`); }
  if (t?.includes('palo-') || t?.includes('traversa-')) { const isUsma = e.team === 'usma'; const hit = e.hitType === 'palo' ? 'Palo' : 'Traversa'; return sanitize(`${min}${hit} ${isUsma ? (ps||'USMA Padova') : opp}`); }
